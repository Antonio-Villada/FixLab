...
export class ChatWidgetComponent implements AfterViewChecked {
  ...
  private readonly authSync = effect(() => {
    ...
    // Si cambió de usuario (o cerró sesión), aislamos el historial.
    if (!loggedIn) {
  ...
  togglePanel(): void {
    ...
    if (!this.panelOpen()) this.error.set(null);
  }
  ...
  enviar(): void {
    ...
    this.chatService.enviarMensaje(t).subscribe({
      ...
      error: (err) => {
        ...
        this.error.set(msg);
        this.mensajes.update((list) => [...list, { emisor: 'bot', texto: msg }]);
        this.persistHistory();
        this.debeHacerScroll = true;
        this.enviando.set(false);
      },
  ...
  /** Cerrar panel y navegar (para enlaces dentro del chat). */
  cerrarYNavegar(url: string): void {
  ...
  private persistHistory(): void {
    ...
    try {
      ...
      // si falla storage (modo privado / cuota), no bloqueamos el chat
    }
  }
