...
public class EmailService {
    ...
    /**
     * Variante para incluir un bloque HTML (ya escapado) en el correo.
     * Útil para cotizaciones donde se necesita tabla/detalle.
     */
    public void enviarNotificacionCambioEstadoReparacionConHtmlExtra(
            String destino,
            String nombreCliente,
            String numeroTicket,
            String estadoAnteriorLegible,
        ...
        String logoBlock = (logo != null && !logo.isBlank())
                ? "<p style=\"margin:0 0 16px 0;\"><img src=\"" + logo + "\" alt=\"FixLab\" width=\"40\" height=\"40\" style=\"display:block;border:0;\" /></p>"
                : "";
        String extraBlock = (extraHtml != null && !extraHtml.isBlank()) ? extraHtml : "";
        String html = "<!DOCTYPE html><html><body style=\"font-family:Segoe UI,Arial,sans-serif;background:#f6f7f9;margin:0;padding:24px;\">"
                + "<table role=\"presentation\" width=\"100%\" cellspacing=\"0\" cellpadding=\"0\"><tr><td align=\"center\">"
                + "<div style=\"max-width:520px;background:#fff;border-radius:8px;padding:28px;text-align:left;border:1px solid #e5e7eb;\">"
                + logoBlock
    ...
    /**
     * Envía un correo HTML con adjunto (factura PDF).
     * Respeta fixlab.mail.log-only.
     */
    public void enviarCorreoHtmlConAdjuntoPdf(
            String destinatario,
            String asunto,
            String htmlBody,
        ...
        if (logOnly) {
            ...
            log.info("Para: {}", destinatario);
            log.info("Asunto: {}", asunto);
            log.info("Adjunto: {} ({} bytes)", nombreArchivoPdf, pdfBytes != null ? pdfBytes.length : 0);
            log.info("Vista previa (solo log): {}", preview);
            log.warn("=== Modo desarrollo: revisa consola / vista previa ===");
            System.out.println("\n========== FIXLAB - CORREO HTML + PDF (modo desarrollo, no enviado) ==========");
            System.out.println("Para: " + destinatario);
            System.out.println("Asunto: " + asunto);
            System.out.println("Adjunto: " + nombreArchivoPdf + " (" + (pdfBytes != null ? pdfBytes.length : 0) + " bytes)");
            System.out.println("--- Vista sin etiquetas (solo para leer en consola) ---");
            System.out.println(preview);
            System.out.println("=============================================================================\n");
            return;
        ...
        try {
            ...
            if (pdfBytes != null && pdfBytes.length > 0) {
                String filename = (nombreArchivoPdf != null && !nombreArchivoPdf.isBlank()) ? nombreArchivoPdf : "factura-fixlab.pdf";
                helper.addAttachment(filename, new ByteArrayResource(pdfBytes), "application/pdf");
            }
            mailSender.send(mimeMessage);
            log.info("Correo HTML con adjunto enviado correctamente a {} (asunto: {})", destinatario, asunto);
        } catch (MessagingException e) {
